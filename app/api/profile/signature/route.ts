...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (file.size > MAX_FILE_SIZE) {
      return NextResponse.json(
        { success: false, error_code: "FILE_TOO_LARGE", message: "File too large. Maximum size: 2MB" },
        { status: 400 }
      );
    }
...
export async function DELETE(): Promise<NextResponse> {
  try {
    ...
    if (!signatureUrl) {
      return NextResponse.json(
        { success: false, error_code: "NO_FILE_TO_DELETE", message: "No signature to delete" },
        { status: 400 }
      );
    }
