...
export async function POST(request: NextRequest) {
  try {
    ...
    if (mode === "replace") {
      ...
      await query(`DELETE FROM rate_overrides WHERE project_id IN (SELECT id FROM projects WHERE user_id = $1)`, [userId]);
      ...
      await query(`DELETE FROM currency_rates WHERE user_id = $1`, [userId]);
    }
      ...
      for (const client of backup.clients) {
        try {
          ...
          const existing = await query(
            ...
            [userId, client.name as string]
          );

          let newClientId: string;

          if (existing.rows.length > 0 && mode === "merge") {
            // Skip existing client in merge mode
            newClientId = existing.rows[0].id as string;
            clientIdMap.set(client.id as string, newClientId);
          } else {
            // Create new client
            ...
            await query(
              ...
              [
                ...
                client.contactName as string || null,
                client.email as string || null,
                client.phone as string || null,
                client.address as string || null,
                client.defaultRate as number || null,
                client.notes as string || null,
                (client.isActive as boolean) ?? true,
                client.createdAt as string || new Date().toISOString(),
                client.updatedAt as string || new Date().toISOString(),
              ]
            );
            stats.clients++;
            clientIdMap.set(client.id, newClientId);
          }
        } catch (error) {
          console.error("Error importing client:", error);
          stats.errors.push({ entity: "client", message: `${client.name}: שגיאה בייבוא` });
        }
      }
    }
    ...
    if (backup.projects && Array.isArray(backup.projects)) {
      for (const project of backup.projects) {
        try {
          ...
          // Check if project with same name and client exists
          const existing = await query(
          ...
          if (existing.rows.length > 0 && mode === "merge") {
            // Skip existing project in merge mode
    ...
    if (backup.customTags && Array.isArray(backup.customTags)) {
      for (const tag of backup.customTags) {
        try {
          ...
          if (existing.rows.length > 0 && mode === "merge") {
            // Skip existing tag in merge mode
            continue;
          }
    ...
    if (backup.currencyRates && Array.isArray(backup.currencyRates)) {
      for (const rate of backup.currencyRates) {
        try {
          // Check if rate with same currencies exists
          const existing = await query(
          ...
          if (existing.rows.length > 0 && mode === "merge") {
            // Skip existing rate in merge mode
            continue;
          }
    ...
    if (backup.rateOverrides && Array.isArray(backup.rateOverrides)) {
      for (const override of backup.rateOverrides) {
        try {
          ...
          // Check if override with same project and tag exists
          const existing = await query(
          ...
          if (existing.rows.length > 0 && mode === "merge") {
            // Skip existing override in merge mode
            continue;
          }
