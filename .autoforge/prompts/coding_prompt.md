...
tail -500 .autoforge/claude-progress.txt
...
Document the SPECIFIC external blocker in `.autoforge/claude-progress.txt`. "Functionality not built" is NEVER a valid reason.
...
Update `.autoforge/claude-progress.txt` with:
...
2. Update .autoforge/claude-progress.txt
...
DO NOT create temporary or helper files in the project root or anywhere outside the project's source directories:

- **No throwaway scripts:** `check-*.mjs`, `check-*.cjs`, `add-*.mjs`, `fix-*.mjs`, `setup-*.mjs`, `verify-*.mjs`, `update-*.mjs`, `create-*.mjs`, `apply-*.mjs`, or similar one-off scripts
- **No session notes:** `FEATURE-*.md`, `feature-*.md`, `BLOCKING-*.md`, `CRITICAL-*.md`, `ACTION-REQUIRED-*.md`, `*.session-notes.txt`
- **No screenshots at root:** Do not save screenshot PNGs to the project root
- **No progress files at root:** Only use `.autoforge/claude-progress.txt` for progress tracking

If you absolutely need a scratch file, put it in `.autoforge/scratch/` and clean it up before committing.

All verification should happen via browser automation tools, not one-off scripts. If you need to test something programmatically, use the project's existing test framework.
